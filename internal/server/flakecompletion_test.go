...
func TestHandlerInitializeAdvertisesCompletionCapability(t *testing.T) {
	...
	if !strings.Contains(string(data), `"completionProvider":{"triggerCharacters":["\"","."],"resolveProvider":true}`) {
		t.Errorf("serialized capabilities = %s, want completionProvider triggerCharacters [\" .] and resolveProvider", data)
	}
}
